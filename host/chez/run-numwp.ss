...
(wp-infer! U (jolt-vector sq usef))

(define nseed (param-num-seeds-for U "user/sq"))
...
(wp-infer! U (jolt-vector sqi usei))
(gate-check "integer caller leaves param generic"
       (jolt-truthy? (param-num-seeds-for U "user/sqi")) #f)
...
(wp-infer! U (jolt-vector esc hof ecl))
(gate-check "escaped fn keeps no double seed"
       (jolt-truthy? (param-num-seeds-for U "user/esc")) #f)
...
(wp-infer! U (jolt-vector mag dist dcl))
(gate-check "param fed a flonum-returning call types :double"
       (jolt-truthy? (param-num-seeds-for U "user/dist")) #t)
...
(wp-infer! U (jolt-vector sq2 use2))
(gate-check "run-passes unboxes a hintless double fn"
       (gate-sub? (emit (run-passes sq2 (make-analyze-ctx "user") U)) "fl*") #t)
