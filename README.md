...
(`libssl`/`libcrypto`, loaded via FFI) for Maven deps — jolt downloads and
resolves those itself and reads each jar in place, with no `curl`, no `unzip`
and no Java. A dependency
