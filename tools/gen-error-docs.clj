...
(defn render []
  (let [kinds (read-string (slurp registry-path))
    ...
    (println)
    (println "The diagnostic's `ex-data` also carries the reference's spelling of its")
    (println "phase and position — `:clojure.error/phase` (`:read-source`,")
    (println "`:compile-syntax-check`, or for what a macro's own run raised")
    (println "`:macro-syntax-check` / `:macroexpansion` with `:clojure.error/symbol`")
    (println "naming the macro), `:clojure.error/line`, `:clojure.error/column` and")
    (println "`:clojure.error/source` (the file, when there is one) — and `Throwable->map`")
    (println "lifts the phase to a top-level `:phase`, so `clojure.main/ex-triage` and")
    (println "everything built on it (REPL `:caught` hooks, test runners, editor")
    (println "middleware) reads a Jolt error the way it reads a JVM one. What a macro's")
    (println "own run threw is the diagnostic's `ex-cause`, so its class survives to the")
    (println "triage. The `:read-source` phase belongs to whoever read the source: the")
    (println "loader, `load-string`, `-e` and `clojure.main/repl` file a read error under")
    (println "it; a program's own `read-string` raises the same positioned diagnostic")
    (println "with no phase, an execution error of that program, as on the JVM.")
    (println)
