...
(defn render []
  (let [kinds (read-string (slurp registry-path))
    ...
    (println "`:clojure.error/source` — and `Throwable->map` lifts the phase to a")
    (println "top-level `:phase`, so `clojure.main/ex-triage` and everything built on it")
    (println "(REPL `:caught` hooks, test runners, editor middleware) reads a Jolt error")
    (println "the way it reads a JVM one.")
    (println)
